...
# draw estimates
pl.figure()
lines_true = pl.plot(data.states[:,0], color='b')
lines_filt = pl.plot(filtered_state_means[:,0], color='r')
cov = pl.fill_between(np.arange(len(filtered_state_means)), filtered_state_means[:,0] - np.sqrt(filtered_state_covariances[:,0,0]), filtered_state_means[:,0] + np.sqrt(filtered_state_covariances[:,0,0]), color="orange", alpha=0.5, label="filt_std")
pl.legend((lines_true[0], lines_filt[0], cov), ('true', 'filtered', 'filt_std'))
pl.show()